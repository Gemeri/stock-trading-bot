...
def _fit_base_classifiers(
    ...
    if "forest_cls" in model_names:
        ...
        pipe = make_pipeline(rf_raw)
        fixed_fit(X_scaled, y_bin, pipe)
...
def train_and_predict(df: pd.DataFrame, return_model_stack=False, ticker: str | None = None, for_selection: bool = False):
    ...
    if any(m in multi_classifier_set for m in ml_models):
        ...
        if "xgboost_multi" in ml_models:
            ...
            fixed_fit(X_multi_scaled, y_encoded, xgb_multi)
        ...
        if "catboost_multi" in ml_models:
            ...
            fixed_fit(X_multi_scaled, y_encoded, cb_multi)
        ...
        if "lightgbm_multi" in ml_models:
            ...
            fixed_fit(X_multi_scaled, y_encoded, lgb_multi)
    ...
    if use_meta_model:
        ...
        for i in range(start_idx, N-1):
            ...
            try:
                ...
                fixed_fit(local_X, local_y, rf_model_meta)
            ...
            try:
                ...
                fixed_fit(local_X, local_y, xgb_model_meta)
            ...
            try:
                cb_model_meta = CatBoostRegressor(iterations=600, depth=6, learning_rate=0.05,
                                                ...
                                                loss_function="RMSE", eval_metric="RMSE",
                                                random_seed=RANDOM_SEED, verbose=False)
                fixed_fit(local_X, local_y, cb_model_meta)
            ...
            try:
                ...
                fixed_fit(local_X, local_y, lgbm_model_meta)
...
def buy_shares(ticker, qty, buy_price, predicted_price):
    ...
    try:
        ...
        if already_short and abs_short_qty > 0:
            ...
            # Use current buy_price as the cover price
...
def sell_shares(ticker, qty, sell_price, predicted_price):
    ...
    try:
        ...
        # 2) No shares OWNED or SHORTED: if USE_SHORTS/USE_SHORT is True, call short_shares
        if pos_qty == 0:
            ...
            logging.info(
                ...
                "USE_SHORTS/USE_SHORT is True; delegating to short_shares to open a new short."
            )
...
def short_shares(ticker, qty, short_price, predicted_price):
    ...
    try:
        ...
        # If already short, avoid duplicate short
        if already_short:
            log_qty = int(abs_short_qty) if USE_FULL_SHARES else round(abs_short_qty, 4)
            logging.info(
                f"[{ticker}] Already short {log_qty} shares. "
        ...
        # If currently long, require an explicit SELL first (no auto-close & re-open)
        if already_long:
            log_qty = int(abs_long_qty) if USE_FULL_SHARES else round(abs_long_qty, 4)
            logging.info(
        ...
        # Flat: open new SHORT (position sizing unchanged)
        if DISCORD_MODE == "on":
            send_discord_order_message(
    ...
    while not SHUTDOWN:
        ...
        if cmd == "turnoff":
            ...
            if run_all:
                ...
                logging.info(
                    f"[{BACKTEST_TICKER}] backtest all: running logic scripts in order: "
                ...
                if {
                    ...
                    logic_modules_to_run = [_get_logic_script_name(str(TRADE_LOGIC))]
            ...
            if skip_data:
                logging.info(
                    f"[{BACKTEST_TICKER}] backtest -r: using existing CSV {csv_filename}"
                )
                if not os.path.exists(csv_filename):
                    logging.error(
                        f"[{BACKTEST_TICKER}] CSV file {csv_filename} does not exist, skipping."
                    )
                    continue
                df = read_csv_limited(csv_filename)
                if df.empty:
                ...
                df.to_csv(csv_filename, index=False)
                logging.info(
                    f"[{BACKTEST_TICKER}] Saved updated CSV with sentiment & features "
            ...
            if 'close' not in df.columns:
                logging.error(
                    f"[{BACKTEST_TICKER}] No 'close' column after feature processing. Cannot backtest."
                )
                continue
            ...
            if train_end < 1:
                logging.error(
                    f"[{BACKTEST_TICKER}] train_end < 1. Not enough data for that test_size."
                )
                continue
            ...
            for logic_module_name in logic_modules_to_run:
                ...
                if cache_state and not backtest_cache_matches(cache_state, cache_meta):
                    logging.info(
                        f"[{BACKTEST_TICKER}] Ignoring cached backtest state for "
                ...
                if approach in ["simple", "complex"]:
                    ...
                    if approach == "simple":
                        ...
                        if not model_stack:
                            logging.error(
                                f"[{BACKTEST_TICKER}] No supported model for simple backtest."
                            )
                            continue
                    else:  # complex
                    ...
                    if resume_data:
                        ...
                        logging.info(
                            f"[{BACKTEST_TICKER}] Resuming backtest for {logic_module_name} "
                    ...
                    if "sub-vote" in ml_models or "sub-meta" in ml_models:
                        ...
                        if resume_data:
                            ...
                            logging.info(
                                f"[{BACKTEST_TICKER}] Resuming signal backtest for {logic_module_name} "
                    ...
                    logging.warning(
                        f"[{BACKTEST_TICKER}] Unknown approach={approach}. "
                ...
                if collecting_regression:
                    ...
                    logging.info(
                        f"[{BACKTEST_TICKER}] Backtest ({approach}) — logic={logic_module_name}, "
                ...
                if collecting_regression:
                    ...
                    logging.info(
                        f"[{BACKTEST_TICKER}] Saved predictions for {logic_module_name} → {out_pred_csv}"
                    )
                    logging.info(
                        f"[{BACKTEST_TICKER}] Saved prediction plot for {logic_module_name} → {out_pred_png}"
                    )
                ...
                logging.info(
                    f"[{BACKTEST_TICKER}] Saved trades for {logic_module_name} → {trade_log_csv}"
                )
                logging.info(
                    f"[{BACKTEST_TICKER}] Saved portfolio series for {logic_module_name} → {port_csv}"
                )
                ...
                if not port_df.empty:
                    ...
                    logging.info(
                        f"[{BACKTEST_TICKER}] Saved portfolio plot for {logic_module_name} → {port_png}"
                    )
            ...
            def _walkforward_shap_importance(X: pd.DataFrame,
                ...
                for i in pbar:
                    ...
                    fixed_fit(X_train, y_train, model)
