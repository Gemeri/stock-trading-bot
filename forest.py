...
def run_sentiment_job(skip_data=False):
    """
    Runs before the trading job.
    For each ticker, this function:
      - Fetches candle data (only to obtain timestamps),
      - Retrieves news articles and computes sentiment,
      - Assigns a sentiment value to each candle row,
      - Saves a CSV containing ONLY the 'timestamp' and 'sentiment' columns.
    This CSV is later used by the trading job.
    """
    for ticker in TICKERS:
        ...
        # Fetch candle data (using BAR_TIMEFRAME) for timestamps only.
        ...
        # (Optionally, if you don’t need extra features here, you can skip add_features/compute_custom_features.)
        # Fetch news sentiments using the existing logic.
        ...
        # Assign a sentiment value to each candle row based solely on timestamp alignment.
        sentiments = assign_sentiment_to_candles(df, news_list)

        # Build a DataFrame containing only the minimal columns needed.
...
def merge_sentiment_from_csv(df, ticker):
    """
    Loads the pre-computed sentiment CSV (generated by run_sentiment_job) for the given ticker,
    then merges the sentiment values into the fresh candle data in 'df' based on nearest timestamps.
    ...
    if not os.path.exists(sentiment_csv_filename):
        ...
        return df  # Return unmodified if not found.
    ...
    # Ensure both DataFrames have their timestamp columns as datetime.
    df_sentiment['timestamp'] = pd.to_datetime(df_sentiment['timestamp'])
    df['timestamp'] = pd.to_datetime(df['timestamp'])

    # Convert the sentiment CSV into a list of dictionaries to mimic the news_list structure.
    news_list = []
    ...
    # Use the existing function to assign sentiments based on the news_list.
...
def check_latest_candle_condition(df: pd.DataFrame, timeframe: str, scheduled_time_ny: str) -> bool:
    """
    Checks whether the most recent candle in df has a UTC open time matching what is expected for
    the given timeframe at the given scheduled NY time.
    
    For example:
      - For 4Hour: if scheduled_time_ny is "20:01", expect "20:00:00+00:00".
      - For 2Hour: if scheduled_time_ny is "10:01", expect "12:00:00+00:00"; if "18:01", expect "20:00:00+00:00".
      - For 1Hour: if scheduled_time_ny is "11:01", expect "12:00:00+00:00"; if "12:01", expect "13:00:00+00:00";
                if "16:01", expect "20:00:00+00:00"; if "17:01", expect "21:00:00+00:00".
                
    Returns True if the latest candle’s timestamp (UTC) exactly matches the expected value;
    if no condition applies, it defaults to the actual value and returns True.
    """
    if df.empty:
        return False

    # Get the last candle's timestamp (assumed to be in UTC)
    last_ts = pd.to_datetime(df.iloc[-1]['timestamp'])
    last_ts_str = last_ts.strftime("%H:%M:%S+00:00")
    
    # Set default: if no condition matches, expected equals actual timestamp
    ...
    if timeframe == "4Hour":
        ...
        return True  # For other timeframes, no check is imposed.
...
def _perform_trading_job(skip_data=False, scheduled_time_ny: str = None):
    for ticker in TICKERS:
        ...
        if not skip_data:
            # Fetch the freshest candle data.
            df = fetch_candles(ticker, bars=N_BARS, timeframe=BAR_TIMEFRAME)
            if df.empty:
...
def setup_schedule_for_timeframe(timeframe: str):
    ...
    schedule.clear()
    for t in times_list:
        # Wrap run_job to pass the scheduled NY time t.
        schedule.every().day.at(t).do(lambda t=t: run_job(t))
        if NEWS_MODE == "on":
            try:
                # Schedule the sentiment job 20 minutes before the trading job.
...
def run_job(scheduled_time_ny: str):
    ...
    # Check if the market is scheduled to be open today.
