...
def run_backtest(
    ...
    candles: pd.DataFrame  # now ignored
) -> str:
    global FEATURE_COLUMNS
    ...
    if not FEATURE_COLUMNS:
        df_full = load_full_csv(TICKERS[0])
        FEATURE_COLUMNS = df_full.columns.drop("timestamp").tolist()

    # 3) Re-load the full DataFrame so we get every feature column
    ticker   = TICKERS[0]
    full_df  = load_full_csv(ticker)
    row      = full_df.loc[full_df["timestamp"] == current_timestamp].iloc[0].copy()
    #    Override with the live prices:
    row["close"]           = current_price
    row["predicted_close"] = predicted_price

    # 4) Build the observation vector in the same order used for training
    obs, _ = model.predict(
        row[FEATURE_COLUMNS].to_numpy(dtype=np.float32),
        deterministic=True
    )

    # 5) Map numeric → string
    if obs == 1 and position_qty == 0:
        return "BUY"
    if obs == 2 and position_qty > 0:
        return "SELL"
    return "NONE"
