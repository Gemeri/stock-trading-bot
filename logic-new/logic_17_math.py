...
def _train_until(ts: Union[pd.Timestamp, str]) -> _ModelCache:
...
def run_logic(current_price: float, _predicted_price: float, ticker: str):
    cache = _train_until(pd.Timestamp.utcnow())
    feat_row = _load_full_data().iloc[-1][FEATURES].values.reshape(1, -1)
...
def run_backtest(
    ...
    candles = _ensure_features(candles)         # <-- NEW
