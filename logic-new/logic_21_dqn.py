...
Core flow:
    ...
    4) Train or load the RL (DQN) model using the CSV data. The agent's state includes
       the features + predicted_price + position quantity, and the action space is:
         [0: BUY, 1: SELL, 2: NONE].
    5) In run_logic, actions are dispatched to 'forest' module functions (buy_shares, sell_shares, etc.).
    6) In run_backtest, simply return the chosen action as a string: "BUY", "SELL", or "NONE".
...
class TradingEnvironment:
    ...
    State includes features + position; actions: BUY, SELL, NONE.
    """
    ...
    def _get_state(self):
        """
        Returns the current feature vector + position.
        """
        if self.current_step >= self.n_steps:
...
def run_backtest(current_price, predicted_price, position_qty, current_timestamp, candles):
    """
    Called in a backtesting environment.
    """
    bar_timeframe = os.environ.get("BAR_TIMEFRAME", "4Hour")
    tf_suffix = convert_bar_timeframe(bar_timeframe)
