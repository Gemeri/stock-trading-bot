...
TICKERS = os.getenv("TICKERS", "TSLA").split(",")  # ALWAYS one ticker, enforced
...
def get_csv_filename(ticker: str) -> str:
    """
    Helper for historical csv files.
    """
...
def load_csv_df(
    ...
    """
    Reads the CSV for a ticker, optionally truncating up to and including `upto_timestamp`.
    """
...
class GAOptimizer:
    """
    Simple Genetic Algorithm for either evolving feature-weighted policies, rule thresholds,
    or small NN weights to seed RL.
    """
    ...
    def _init_population(self) -> List[np.ndarray]:
        """
        Each individual is a vector of weights for each feature + biases for simple rule.
        """
    ...
    def _evaluate_individual(self,
        ...
        """
        - Simple policy: action = sign(w*state)
        - Fitness: net profit - 0.5*max_drawdown - 0.1*n_trades
    ...
    def optimize(self, df: pd.DataFrame) -> Dict:
        """
        Main entry: runs evolution, sets self.best_params to best individual's weights
        Returns dict with best params and metrics.
        """
...
class RLEnvironment:
    """
    Simple RL environment: feeds state, receives action, provides reward (PnL step)
    """
    ...
    def step(self, action:int) -> Tuple[np.ndarray, float, bool, Dict]:
        """
        Executes action, returns (next_state, reward, done, info)
        """
        if self.done:
...
class RLAgent:
    """
    Implements DQN (tabular or function approx) with MLP and Experience Replay.
    For brevity, a simple linear Q-table is shown here.
    Use PyTorch or stable-baselines3 to extend with real MLP/DDQN/PPO!
    """
    ...
    def train(self, df: pd.DataFrame, n_episodes:int = 100, batch_size:int = 32, gamma:float = 0.99) -> "RLAgent":
        """
        Trains RL agent on the walkforward data.
        For real usage, use MLP and torch with batches, targets, etc!
        Here, a very simple approach: for each episode, run through environment, do policy eval, update weights.
        """
    ...
    def act(self, state: np.ndarray) -> str:
        """
        Given a state vector (features+position) returns "BUY"|"SELL"|"NONE"
        """
...
def _get_feature_cols(df: pd.DataFrame) -> List[str]:
    """
    Helper to get list of all features, excluding target variables.
    """
...
def run_backtest(current_price: float,
    ...
    """
    Backtesting mode. At each step:
    - Trains GA and RL on csv up to current_timestamp (NO future data)
    - Returns action on current state: "BUY"|"SELL"|"NONE"
    """
    try:
...
def run_logic(current_price: float,
    ...
    """
    Live mode.
    - Loads entire CSV, runs GA + RL, decides action, executes trade via forest.api
    """
    try:
...
# === If run standalone, test backtest API with example parameters ===
if __name__ == "__main__":
    # Example test - replace with your own test csv or proper CLI wrapper
    logger.info("======== Module test: ga_rl_trade_logic.py ========")
    try:
        # Simulate a backtest step
        TICKER = TICKERS[0]
        df = load_csv_df(TICKER)
        N = len(df)
        if N < 20:
            logger.warning("CSV too short for test.")
            sys.exit(1)
        test_idx = int(N*0.75)
        test_candles = df.iloc[:test_idx+1].copy()
        ts = test_candles.iloc[-1]["timestamp"]
        cur_price = float(test_candles.iloc[-1]["close"])
        pred_price = float(test_candles.iloc[-1]["predicted_close"])
        position_qty = 0.0
        logger.info(f"Test run_backtest...")
        ret = run_backtest(cur_price, pred_price, position_qty, ts, test_candles)
        print(f"run_backtest() result: {ret}")
    except Exception as e:
        logger.error(f"Main test failed: {str(e)}", exc_info=True)