...
class RuleSet:
    """
    Represents a trading rule candidate as evolved by GA.
    Each RuleSet is a combination of used features, operators, and thresholds.
    """
    ...
    def decide(self, features: Dict[str, Any], sentiment: float) -> int:
        """Apply rule logic to feature dict and sentiment value:
        Returns:
            +1 : buy, -1 : sell, 0 : none
        """
        mask_vals = []
...
def fitness(rule: RuleSet, df: pd.DataFrame) -> float:
    """
    Backtest the rule on df (uses only known columns and step-by-step).
    Returns:
        Sharpe ratio * CAGR * (1 - max drawdown)
        Penalizes high drawdown and low profit; reward positive skew/profit.
    """
    startcash = 100000.0
...
def GA_optimize(df: pd.DataFrame, features: List[str], ngen: int = 20, pop: int = 18) -> RuleSet:
    """Basic Genetic Algorithm: evolve a pool of RuleSets."""
    # Init population
...
class RLQAgent:
    """
    Simple Q-learning agent to fine-tune thresholds and rules on top of GA.
    Uses a 3-state encoding for each signal: {-1, 0, +1} and RL for action selection.
    """
    ...
    def train(self, df: pd.DataFrame, rule: RuleSet, episodes=3):
        """
        Trains Q agent on data up to current window.
        """
...
def run_logic(current_price, predicted_price, ticker):
    """
    Integrates previously trained GA+RL models to generate action: BUY, SELL, NONE.
    Uses both structural rules and sequence signal fine-tuned by RL.
    """
    import sys
...
def run_backtest(current_price, predicted_price, position_qty, current_timestamp, candles):
    """
    At each step, load ticker CSV up to (and incl) current_timestamp,
    reoptimize (unless rule/RL is persistent in memory for this window)
    and output "BUY", "SELL" or "NONE".
    """
