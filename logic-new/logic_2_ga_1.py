...
def tree_to_action(root_output):
    """Convert program output to discrete action."""
    # If the tree output is >0.5: BUY; <-0.5: SELL; else NONE.
...
def walk_forward_fitness(individual, toolbox, X, y_price, y_pred, timestamps):
    ...
    # "Evaluate" on validation, but train on train set (evolve on train only!)
    # But DEAP fitness call gets all history: so, measure fitness on train, validate for GA selection. We'll average both into one fitness.
...
def simulate_trading(individual, toolbox, X, closes, predicted, timestamps, initial_cash=100000.0):
    """
    Simulate stepwise execution for an individual on a given X.
    Returns (final_profit, sharpe, max_drawdown, turnover)
    All positions are long-only, no margin/shorts.
    """
    ...
    # To annualize, assume ~252*X per year (varies by timeframe); use stddev of returns
    # We'll use returns in percent of initial.
    if len(returns) > 1:
...
def prepare_ga_dataset(up_to_timestamp):
    """Prepare feature targets and normalization up to a certain timestamp (inclusive)."""
    ...
    # Compose features and outputs
    X = candles[FEATURE_COLUMNS].values.astype(float)
    # y_price is the true next-close; y_pred is the model's predicted next-close (the 'predicted_close' column)
    ...
    # Fit scaler to features so that each feature is zero mean/unit std in the window.
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)
    return X_scaled, y_price, y_pred, timestamps, scaler

def evolve_ga(X, y_price, y_pred, timestamps):
    """Run the full GA, return the best individual (strategy)."""
    ...
    for g in range(GA_N_GEN):
        ...
        valid_fits = [f for f in fits if f > -1e11] # exclude penalized
        if valid_fits:
...
def run_logic(current_price, predicted_price, ticker):
    """
    Executes the GA-evolved strategy.
    - Loads champion strategy from disk.
    - Evaluates the best GP tree on the current normalized feature vector.
    - Buys all available cash if strategy emits BUY, Sells all if SELL, else NONE.
    """
    # API/positions
    try:
    ...
    # Compose vector (with backward-compatible defaults)
    features = []
    for c in FEATURE_COLUMNS:
        val = latest[c] if c in latest else 0.0
        features.append(val)
    # Overwrite these with up-to-date on-the-spot inputs:
    # Assume 'close' is the current market price, 'predicted_close' is predicted_price.
    try:
...
def run_backtest(current_price, predicted_price, position_qty, current_timestamp, candles):
    """
    Stepwise walk-forward backtest for the GA-evolved strategy.
      - At each bar, fit GA up to and including current_timestamp (never peeking ahead!).
      - Predicts and returns "BUY", "SELL", or "NONE" for this bar.
    
    Parameters:
        current_price:      [float] - the close at the current timestamp
        predicted_price:    [float] - the model's forecast for next-candle
        position_qty:       [float] - current holdings (long-only!)
        current_timestamp:  [pd.Timestamp] or compatible
        candles:            [DataFrame] - the sub-data up to the current backtest bar
    Returns: string "BUY" / "SELL" / "NONE"
    """
    # Prepare historical features strictly up to current_timestamp
    # current_timestamp might be string or pd.Timestamp
    ...
    # Compose feature vector for current timestep
    # candles: should be a DataFrame up through current_timestamp
    this_row = candles.iloc[-1]  # last row is current time
    features = []
    for c in FEATURE_COLUMNS:
        features.append(this_row.get(c, 0.0))
    # Patch with actual prices for this step (as in live)
    try:
        idx_close = FEATURE_COLUMNS.index('close')
        features[idx_close] = current_price
        idx_pred = FEATURE_COLUMNS.index('predicted_close')
