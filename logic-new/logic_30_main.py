...
ML_MODEL = os.getenv('ML_MODEL', 'forest')  # 'forest' or 'xgboost'
...
def train_rf_model(df):
    """
    Train a model on the historical data to predict the close price.
...
def create_rl_env(df, rf_model, rf_features):
    """
    Computes the predicted_price column using the model,
    then creates the TradingEnv using the ENABLED_FEATURES and the predicted price.
    """
    df = df.copy()
    # Predict the close price from the model and add it as a column
    X_rf = df[rf_features]
...
def train_rl_model(env):
    ...
    """
    model = PPO("MlpPolicy", env, device="cpu", verbose=0, gamma=0.99, learning_rate=0.0003)
