"""
logic_ga_optimized.py
───────────────────────────────────────────────────────────────────────────────
Genetic‑Algorithm trading strategy generator (BUY/SELL/NONE, long‑only).

✓   run_logic()     – live trading; loads a pre‑trained “champion”.
✓   run_backtest()  – re‑trains GA up to each timestamp, then decides.
✓   GA config       – 100×50 population, tournament(3), CX 0.8, MUT 0.1.
✓   Fitness         – profit ↑, Sharpe ↑, max‑drawdown ↓, turnover ↓.
✓   Features        – full vector you provided (see FEATURES list).
✓   No future‑leak  – training slice restricted to data ≤ current_timestamp.
───────────────────────────────────────────────────────────────────────────────
"""
...
# ── Add this near the top with the other DEAP imports ────────────────────────
...
MUT_RATE: float     = 0.1
TOURN_SIZE: int     = 3
RISK_FREE: float    = 0.0        # for Sharpe
...
FEATURES: List[str] = [
    ...
    "vwap", "macd_line", "macd_signal", "macd_histogram",
...
try:
    from forest import api, buy_shares, sell_shares        # noqa: E402
    ...
    class _FakeAPI:                                        # noqa: D401
        ...
        def get_position(self, tic):                       # noqa: D401
    ...
    def buy_shares(ticker, qty, price, predicted_price):   # noqa: D401
        api.update(ticker, qty)
        logger.info(f"[{ticker}] ***STUB BUY {qty} @ {price} "
                    f"– next_pred {predicted_price}")

    def sell_shares(ticker, qty, price, predicted_price):  # noqa: D401
        api.update(ticker, 0)
...
TICKER = TICKERS[0]  # spec: always one ticker
...
def _setup_deap():
    """
    Register *unique* DEAP types for this GA so we never clash with other
    modules that may have defined GP‑based individuals.  We also create an
    alias 'creator.Individual' that points to the same list subclass, so any
    legacy code that expects that name (e.g. the lines you added later) will
    still work without importing gp.PrimitiveTree.
    """
    ...
    global toolbox                       # already instantiated above
...
def _load_data(ticker: str) -> pd.DataFrame:
    """Lazy‑load CSV and cache (parse timestamp)."""
...
def _split_train_val(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """80‑20 chronological split (walk‑forward)."""
    split_idx = int(len(df) * 0.8)
    return df.iloc[:split_idx], df.iloc[split_idx:]


def _standardize(df: pd.DataFrame, ticker: str) -> np.ndarray:
    """Return standardized feature matrix; cache scalers per ticker."""
...
def _simulate(
    ...
    """
    Run strategy over df, returning:
        net_profit, sharpe, max_dd, turnover
    """
    cash   = INITIAL_CASH
    shares = 0.0
    equity_curve = []
...
def _fitness_generator(df_train: pd.DataFrame, ticker: str):
    """Closure so DEAP can use cached data without repeated standardization."""
    ...
    def evaluate(ind: List[float]):
        ...
        fitness = (
            pnl
            + 1_000 * sharpe          # Sharpe weighted
            - 1_000 * max_dd          # penalize %
            - 100  * turnover         # penalize freq
        )
        return (fitness,)
    return evaluate
...
def _train_ga(df: pd.DataFrame, ticker: str) -> creator.GA_Individual:
    """
    Evolve a GA population on *df* up to the current timestamp and return
    the best individual.  The function is self‑contained and does **NOT**
    rely on any global toolbox, so it cannot conflict with other DEAP
    configurations (e.g. gp.PrimitiveTree) that your project may have
...
def run_logic(current_price: float, predicted_price: float, ticker: str):
    """
    Live trading entry point (long‑only).

    Parameters
    ----------
    current_price : float
    predicted_price : float  – model’s next‑candle forecast (for logging)
    ticker : str
    """
    # ------------------------------------------------------------------ load
    ...
    # we only know current_price & predicted_price live; fall back to simple
    # rule: use champion’s thresholds on two‑feature vector [price, pred]
    # weight slice limited to first two features for live mode
    w = np.array(champ[:2])
...
def run_backtest(
    ...
    """
    Back‑testing entry point.

    – Retrains GA **each call** on historical data up to current_timestamp.
    – Uses champion thresholds to decide BUY/SELL/NONE at this step.

    Parameters
    ----------
    current_price      : float
    predicted_price    : float
    position_qty       : float   – current position (long shares)
    current_timestamp  : datetime
    candles            : pd.DataFrame  – only back‑test slice (ignored for train)
    ...
    if len(df_hist) < 100:          # safety for very early periods
        return "NONE"
