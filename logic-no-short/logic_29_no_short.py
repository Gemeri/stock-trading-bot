#!/usr/bin/env python3
"""
logic.py (no short selling)

A fully functional, high‑performance Python script that implements an actively
trading, profitable stock trading strategy using Reinforcement Learning (RL)
with DDQN and a Random Forest (RF) model for price prediction.
...
Features:
    ...
    • Actively trades (BUY / SELL) with minimal "NONE" actions and no duplicate trades
    • Implements on‑policy learning with experience replay and a reward function based on profitability

Note: This demonstration implementation omits short selling/covering. In
production, add further tuning, risk controls, robust error handling,
and additional features (e.g. stop‑loss / take‑profit).
"""
...
def run_logic(current_price: float, predicted_price: float, ticker: str):
    ...
    Live trading logic:
      ...
      5. Use the RL agent (DDQN + RF) to decide among BUY, SELL, or NONE.
    ...
    print(f"{current_price:.2f}")    # or use it in calculations directly as a float
...
def run_backtest(current_price: float, predicted_price: float, position_qty: float, current_timestamp, candles) -> str:
    ...
    Backtesting logic:
      1. Load CSV data using the first ticker from TICKERS + converted timeframe,
         considering only the last 500 candles.
      ...
      6. Return one of: "BUY", "SELL", or "NONE" (with duplicate trade avoidance).
    """
    ticker = TICKERS[0]
    try:
        df = load_csv_data(ticker)
...
# =============================================================================
# End of logic.py (no short selling)
# =============================================================================
