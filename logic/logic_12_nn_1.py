...
FEATURES = [
    ...
    'macd_line', 'macd_signal', 'macd_histogram', 'ema_9', 'ema_21', 'ema_50', 'ema_200',
    'adx', 'rsi', 'momentum', 'roc', 'atr', 'obv', 'bollinger_upper', 'bollinger_lower',
...
def run_backtest(current_price, predicted_price, position_qty, current_timestamp, candles, ticker=None):
    if ticker is None:
        ticker = candles.get("ticker", [None])[0]
    csv_path = get_csv_filename(ticker)
    if not os.path.exists(csv_path):
        logger.error(f"Data CSV not found: {csv_path}")
        return "NONE"
