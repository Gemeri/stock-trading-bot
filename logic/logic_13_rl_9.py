...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df: pd.DataFrame, memory_summary: np.ndarray = None):
        ...
        # build observation space (size never changes)
    ...
    def reset(self):
        ...
        self.position      = 0
        self.cash          = 1e6
        self.prev_value    = 1e6                             # <-- keep last portfolio value
    ...
    def step(self, action):
        ...
        reward          = portfolio_value - self.prev_value   # <-- incremental reward
...
def run_logic(current_price: float, predicted_price: float, ticker: str):
    ...
    numeric_full = (
        full_df.select_dtypes(include=[np.number]).dropna().reset_index(drop=True)
    )

    # --- ensure the forecast column exists in history -----------------
    if 'forecast_edge' not in numeric_full.columns:
        numeric_full['forecast_edge'] = 0.0            
    ...
    # --- inject current forecast edge into the latest row -------------
    latest.loc[:, 'forecast_edge'] = predicted_price - current_price
...
def run_backtest(current_price: float,
    ...
    cand_num = (
        candles.select_dtypes(include=[np.number]).dropna(axis=1, how='any')
    )
    latest = cand_num.iloc[[-1]].astype(np.float32)

    # --- inject forecast edge for this step ---------------------------
    latest.loc[:, 'forecast_edge'] = predicted_price - current_price
    # 2) Initial warm-up
    if _backtest_model is None:
        full = pd.read_csv(get_csv_filename(TICKERS[0]), parse_dates=["timestamp"])
        hist = (
            full.select_dtypes(include=[np.number]).dropna().reset_index(drop=True)
        )

        # --- add forecast column to history ---------------------------
        if 'forecast_edge' not in hist.columns:
            hist['forecast_edge'] = 0.0
        _backtest_memory = MemorySystem()
        mem_zero         = np.zeros(_backtest_memory.dim, dtype=np.float32)
