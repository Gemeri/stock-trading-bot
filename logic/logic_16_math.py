...
FEATURES = [
    ...
    'macd_line', 'macd_signal', 'macd_histogram', 'rsi', 'momentum', 'roc', 'atr', 'obv',
    'bollinger_upper', 'bollinger_lower', 'ema_9', 'ema_21', 'ema_50', 'ema_200', 'adx',
    'lagged_close_1', 'lagged_close_2', 'lagged_close_3', 'lagged_close_5', 'lagged_close_10',
    'candle_body_ratio', "predicted_close", 'wick_dominance', 'gap_vs_prev', 'volume_zscore',
    'atr_zscore', 'rsi_zscore', 'adx_trend', 'macd_cross', 'macd_hist_flip', 'day_of_week',
...
def get_historical_data_up_to(timestamp):
    df = load_full_csv()
    df = df[df['timestamp'] <= parse_timestamp(timestamp)]
    df = df.reset_index(drop=True)
    return df

def get_data_for_live():
    return load_full_csv()
...
def run_backtest(current_price, predicted_price, position_qty, current_timestamp, candles):
    ...
    hist = get_historical_data_up_to(current_timestamp)
