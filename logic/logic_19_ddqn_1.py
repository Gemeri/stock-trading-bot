...
def filter_features(df):
    ...
    possible_feats = [
        'timeframe','open','high','low','close','volume','vwap','price_change',
        'high_low_range','log_volume','sentiment','price_return','candle_rise',
        'body_size','wick_to_body','macd_line','rsi','momentum','roc','atr',
        'hist_vol','obv','volume_change','stoch_k','bollinger_upper',
...
# -----------------------------------------------------------------------
def get_or_create_ddqn_agent(ticker, bar_suffix, state_size, action_size=4):
    global DDQN_AGENTS
...
def offline_train_ddqn(ticker, bar_suffix, df_filtered, episodes=1):
    ...
    agent = get_or_create_ddqn_agent(ticker, bar_suffix, state_size, action_size=4)
...
def run_logic(current_price, predicted_price, ticker):
    ...
    offline_train_ddqn(ticker, bar_suffix, df_filtered, episodes=1)
    ...
    agent = get_or_create_ddqn_agent(ticker, bar_suffix, state.shape[0], action_size=4)
    action_idx = agent.act(state)
    action_str = ACTIONS[action_idx]
    ...
    if action_str == "BUY":
        ...
        if old_pos == 1:
            sell_shares(ticker, abs(int(position_qty)) or 1)
            new_pos = 0
        else:
            action_str = "NONE"
    elif action_str == "SHORT":
        ...
        if old_pos == -1:
            close_short(ticker, abs(int(position_qty)) or 1)
            new_pos = 0
        else:
            action_str = "NONE"
...
def run_backtest(current_price, predicted_price, position_qty, current_timestamp, candles):
    ...
    agent = get_or_create_ddqn_agent(first_ticker, bar_suffix, state.shape[0], action_size=4)
    action_idx = agent.act(state)
    action_str = ACTIONS[action_idx]
