...
TB_H = int(os.getenv("TB_H", "5"))                        # horizon in bars
TB_THETA_UP = float(os.getenv("TB_THETA_UP", "0.01"))     # +1% default
...
DIVERSITY_CORR_MAX  = float(os.getenv("DIVERSITY_CORR_MAX", "0.92"))

# Early stopping
EARLY_STOP_PATIENCE = int(os.getenv("EARLY_STOP_PATIENCE", "25"))

# Repro
random.seed(int(os.getenv("EVOBOT_SEED", "1337")))
np.random.seed(int(os.getenv("EVOBOT_SEED", "1337")))
...
def node_to_code(node, indent=1):
    ...
    BUY/SELL expressions are built by walking the GP tree and collecting
    the path conditions that end in each action (disjunction of conjunctions).
    """
    space = "    " * indent
...
def logic_to_live_code(node, indent=1, meta_cfg: Optional[dict]=None):
    """
    Live-trading version of the flat decision chain with an optional
    ...
    No fees, no lagging, trading at bar i.
    """
    # base indents
    space = " " * 4 * indent
    i1 = " " * 4 * (indent + 1)
...
def load_csv_data(ticker: str, current_timestamp=None) -> pd.DataFrame:
    """
    Load the CSV for `ticker`. If `current_timestamp` is provided (datetime-like),
    return only rows with timestamp <= current_timestamp (inclusive).
    No lagging; features are used as-is (per user request).
    """
...
def triple_barrier_labels(close: np.ndarray, up: float, dn: float, h: int) -> np.ndarray:
    """
    close: 1D array of prices
    Returns y in {+1, 0, -1} per bar i based on barriers within next h bars.
    No effect on live features; used only for training/validation labeling.
    """
    n = len(close)
...
def simulate_with_meta(df: pd.DataFrame,
    ...
    """
    Simulate trading ONLY on indices in idx.
    s_hat: base direction per bar (+1 BUY, -1 SELL, 0 NONE)
    meta: logistic gate trained on train (None => always take)
    Returns: equity_curve, n_buys, n_sells, trade_pnls, holding_lengths
    """
    cash = 10000.0
    position_qty = 0
    position_price = 0.0
...
def get_base_signals(candidate: Candidate, df: pd.DataFrame, idx: np.ndarray) -> np.ndarray:
    """
    Use candidate's backtest function (action-only) to compute base directions s_hat.
    BUY->+1, SELL->-1, NONE->0. Ignores position state for signal direction.
    """
...
def evaluate_candidate(candidate: Candidate,
    ...
    """
    Train meta on TRAIN; compute composite J on VALIDATION.
    """
