...
# ---- Env & Globals -----------------------------------------------------------
# Timeframe config (exactly as you provided)
...
# XGBoost hyperparameters (can be overridden via env)
...
XGB_MIN_CHILD_WEIGHT  = float(os.getenv("XGB_MIN_CHILD_WEIGHT", "1.0"))
XGB_EARLY_STOP_ROUNDS = int(os.getenv("XGB_EARLY_STOP_ROUNDS", "25"))
XGB_RANDOM_STATE      = int(os.getenv("XGB_RANDOM_STATE", "42"))
XGB_PROBA_THRESHOLD   = float(os.getenv("XGB_PROBA_THRESHOLD", "0.60"))  # agreement threshold

# Trading / risk params
STOP_LOSS_PCT         = float(os.getenv("STOP_LOSS_PCT", "0.02"))  # 2% trailing stop by default
MIN_TRAIN_ROWS        = int(os.getenv("MIN_TRAIN_ROWS", "300"))    # minimum rows to train
VAL_SPLIT             = float(os.getenv("VAL_SPLIT", "0.2"))       # validation split
...
def _train_two_xgb(df_train: pd.DataFrame, df_valid: pd.DataFrame, feat_cols: list):
    ...
    params = dict(
        ...
        n_jobs=int(os.getenv("XGB_N_JOBS", str(os.cpu_count() or 4))),
        verbosity=0
    )
...
def _decide_action(
    ...
    """
    Core decision logic:
      1) Enforce trailing stop if holding.
      2) Train up/down models on data up to cutoff_ts (or all).
      3) Infer signal by agreement.
      4) If BUY and flat: set entry & initial stop.
         If SELL and holding: clear entry/stop.
    Returns (action, telemetry)
    """
...
def run_logic(current_price, predicted_price, ticker):
    """
    Live trading:
      - Ignores predicted_price (per requirement).
      - Uses account & position from `forest.api`.
      - Uses the same decision logic as backtest including cache & trailing stop.
    """
...
def run_backtest(current_price, predicted_price, position_qty, current_timestamp, candles, ticker):
    """
    Backtest:
      - Uses identical decision logic as live (including cache & trailing stop).
      - Respects 'current_timestamp': trains ONLY on data <= current_timestamp.
      - Ignores predicted_price and candles (per requirement).
