...
BAR_TIMEFRAME = os.getenv("BAR_TIMEFRAME", "4Hour")  # default to 4H candles
TIMEFRAME_MAP = {
...
RETRAIN_EVERY = 1
PER_CLASS_AUC_THRESHOLD = 0.55
GINI_THRESHOLD = 0.09
ENTROPY_THRESHOLD = 0.90
VAL_FRACTION = 0.2
LOOKAHEAD_HORIZONS = [2, 4, 8, 16]
QUANTILE_WINDOW = 32
...
def get_csv_filename(ticker: str) -> str:
    return os.path.join(DATA_DIR, f"{ticker}_{CONVERTED_TIMEFRAME}.csv")
...
def _get_pred_target_for_timestamp(
    ...
    return pred_target, models_up, models_down, models_stable
...
def _decide_trade_action(
    ...
    pred_target, models_up, models_down, models_stable = _get_pred_target_for_timestamp(
        df=df,
