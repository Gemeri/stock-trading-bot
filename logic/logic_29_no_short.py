...
STATE_SIZE = sample_feature_count + 2
...
def run_logic(current_price: float, predicted_price: float, ticker: str):
    ...
    Live trading logic:
      ...
      3. Create RL state (features + predicted_price + open position).
      4. Retrieve current open position from live API.
      5. Use the RL agent to decide BUY, SELL, or NONE.
      6. Execute the chosen trade via the API (avoiding duplicates).
...
def run_backtest(current_price: float, predicted_price: float, position_qty: float) -> str:
    """
    Backtesting logic:
      1. Load CSV data and consider last 500 candles.
      2. Filter out disabled features.
      3. Create RL state (features + predicted_price + position_qty).
      4. Use RL agent to determine BUY, SELL, or NONE.
      5. Apply a confidence threshold to avoid minor moves.
