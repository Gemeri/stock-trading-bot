...
FEATURE_COLUMNS = tools.FEATURES
...
def prepare_ga_dataset(up_to_timestamp, ticker):
    candles = load_all_candles(ticker)
...
def run_backtest(current_price, predicted_price, position_qty, current_timestamp, candles, ticker):
    ...
    X, y_price, y_pred, timestamps, scaler = prepare_ga_dataset(ts, ticker)
