...
REQUIRED_FEATURES: List[str] = tools.FEATURES
...
logger = logging.getLogger("QRDQN")
...
class DirectionalEnv(gym.Env):
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        if action == 0:
            ...
            trade_dir = 1.0 if action == 1 else -1.0
...
def _make_env(df: pd.DataFrame, feature_cols: List[str]) -> Monitor:
    env = DirectionalEnv(df=df, config=EnvConfig(feature_cols=feature_cols))
    return Monitor(env)
...
def _prepare_observation_for_timestamp(
    ...
    env: DirectionalEnv = env_monitor.env
    scaler = env.scaler
    env_feature_order = list(env.feature_cols)
...
def run_logic(current_price: float, predicted_price: float, ticker: str) -> None:
    ...
    # Load full CSV
    df = _ensure_and_prepare_df(get_csv_filename(ticker))
    ...
    if not pd.api.types.is_datetime64_any_dtype(dfx['timestamp']):
        ...
        dforig = pd.read_csv(get_csv_filename(ticker))
...
def run_backtest(
    ...
    # Load full CSV
    df_full = _ensure_and_prepare_df(get_csv_filename(ticker))
