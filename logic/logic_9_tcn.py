...
USE_TIME_DECAY = 1
TIME_HALF_LIFE = 250
TIME_W_CLIP_MIN = float(os.getenv("TCN_TIME_W_CLIP_MIN", "1e-3"))
...
def _time_decay_weights(n_samples: int, half_life: int) -> np.ndarray:
    ages = np.arange(n_samples - 1, -1, -1)  # oldest -> big, newest -> 0
    w = np.exp(-np.log(2) * ages / max(1, half_life))
    return np.clip(w, TIME_W_CLIP_MIN, 1.0).astype(np.float32)
...
def _prepare_training_matrices(
    ...
    if USE_TIME_DECAY:
        tw = _time_decay_weights(len(X_train), TIME_HALF_LIFE)  # (N_train,)
        w_train = w_train * tw[:, None]  # broadcast to (N_train, 3)
