...
# ===== Add/replace in your CONFIG section =====
TCN_CACHE_DIR = os.getenv("TCN_CACHE_DIR", "tcn-cache")  # << new cache root
os.makedirs(TCN_CACHE_DIR, exist_ok=True)
RETRAIN_EVERY = int(os.getenv("TCN_RETRAIN_EVERY", "10"))  # << candles between retrains

# TQDM (nice-to-have). Falls back to plain range if not installed.
try:
    from tqdm import trange
except Exception:
    def trange(n, **kwargs):  # noqa: D401
        """Fallback for tqdm.trange."""
        return range(n)
...
LOOKBACK = int(os.getenv("TCN_LOOKBACK", "256"))
