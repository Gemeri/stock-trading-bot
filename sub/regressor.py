...
# Keep same feature set you already have in CSVs
FEATURES: List[str] = [
...
def _target_col(h: int) -> str:
    # We now train/predict RAW CLOSE LEVELS at t+h.
    return f"target_h{h}"


def compute_targets(df: pd.DataFrame) -> pd.DataFrame:
    """
    Raw level targets: target_h{h} = close_{t+h}  (no percentage/return scaling).
    """
    d = df.copy()
    close = pd.to_numeric(d["close"], errors="coerce").astype(float)
    for h in MULTI_HORIZONS:
        d[_target_col(h)] = close.shift(-h)
...
def fit(X_train: np.ndarray, Y_train: np.ndarray):
    """
    Fit CatBoost regressor on RAW CLOSE targets (levels).
    """
    logging.info("FIT CatBoostRegressor – training on raw close targets")
...
def predict(model, X: np.ndarray) -> np.ndarray:
    """
    Predict RAW CLOSES per horizon.
    Output units: price (same units as 'close').
    """
    X = _as_2d_x(X)
    raw = model.predict(X)
    ...
    print("Running PREDICT on regressor.py")
    return preds


# ---------- Optional CLI walk-forward (now in LEVEL space) ----------
@dataclass
...
def _walk_forward(df: pd.DataFrame, cfg: WalkForwardConfig, output_path: str) -> pd.DataFrame:
    """
    Walk-forward demo that saves predicted and actual RAW CLOSES (levels).
    """
    logging.info("Walk-forward (multi-horizon reg on raw closes; output saved)")
    ...
    for t in tqdm(range(start, end), desc="Walk-forward (multi-reg)"):
        ...
        Y_tr = tr_slice[tgt_cols].values  # raw closes at horizons
        model = fit(X_tr, Y_tr)

        yhat_lvl = np.asarray(predict(model, row_feats), dtype=float)[0]  # raw closes
        rec = {"t": t}
        for i, h in enumerate(MULTI_HORIZONS):
            rec[f"pred_h{h}"] = float(yhat_lvl[i]) if i < len(yhat_lvl) else float('nan')
            actual_lvl = close[t + h] if t + h < len(close) else np.nan
            rec[f"actual_h{h}"] = float(actual_lvl) if np.isfinite(actual_lvl) else float('nan')
    ...
    if mask_h1.sum() > 0:
        ...
        print(f"Saved to {output_path} | (close) h=1 MAE={mae:.6f} RMSE={rmse:.6f}")
    else:
